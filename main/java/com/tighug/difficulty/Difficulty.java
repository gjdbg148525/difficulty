    ...
    public static class Event {
        ...
        public static void onWorldTickEvent(TickEvent.WorldTickEvent event) {
            ...
            if (!event.world.isClientSide()) {
                double day1 = (event.world.getDayTime() / 24000f % 2147483647L) * Config.DOUBLE_VALUE.get() + Config.INITIAL_VALUE.get();
                day = (float) Utils.clamp(day1,0, Config.MAX_VALUE.get());
            }
        }
        ...
        public static void onWorldLoadEvent(WorldEvent.Load event) {
            if (event.getWorld() instanceof ServerWorld) {
                double day1 = ((((ServerWorld) (event.getWorld())).getDayTime() / 24000f % 2147483647L) * Config.DOUBLE_VALUE.get() + Config.INITIAL_VALUE.get());
                day = (float) Utils.clamp(day1,0, Config.MAX_VALUE.get());
            }
        }
                ...
                if (i > 0) {
                    ...
                    if (entity instanceof ServerPlayerEntity) {
                        ...
                        if (f > 1) {
                            ...
                            Arrays.stream(EquipmentSlotType.values()).filter(equipmentSlotType -> equipmentSlotType.getType() == EquipmentSlotType.Group.ARMOR)
                                    .forEach(equipmentSlotType -> {
                                        ItemStack itemBySlot = entity.getItemBySlot(equipmentSlotType);
                                        if (!itemBySlot.isEmpty() && itemBySlot.isDamageableItem()) map.put(equipmentSlotType, itemBySlot);
                                    });
                            if (!map.isEmpty()) {
        ...
        public static void onLivingUpdateEvent(LivingEvent.LivingUpdateEvent event) {
            LivingEntity entity = event.getEntityLiving();
            if (!MAP.isEmpty() && !entity.level.isClientSide()) {
                MAP.getOrDefault(entity, () -> {}).run();
                MAP.remove(entity);
            }
        }
        ...
        public static float getDay() {return day;}

        private static int getInt(@NotNull LivingEntity entity) {
